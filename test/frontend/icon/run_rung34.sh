...
for icn in "$CORPUS"/t*.icn; do
  ...
  "$DRIVER" -jvm "$icn" -o /tmp/t34.j 2>/dev/null
  java -jar "$JASMIN" /tmp/t34.j -d /tmp/ 2>/dev/null
  cls=$(grep -m1 '\.class' /tmp/t34.j | awk '{print $NF}')
...
echo "--- rung34: $PASS pass, $FAIL fail, $XFAIL xfail ---"
[ $FAIL -eq 0 ]
