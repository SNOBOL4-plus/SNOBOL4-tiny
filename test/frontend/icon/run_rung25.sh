...
for icn in test/frontend/icon/corpus/rung25_global/t*.icn; do
  ...
  java -jar src/backend/jvm/jasmin.jar /tmp/t25.j -d /tmp/ 2>/dev/null
  for rj in /tmp/*\$*.j; do [ -f "$rj" ] && java -jar src/backend/jvm/jasmin.jar "$rj" -d /tmp/ 2>/dev/null; done
  cls=$(grep -m1 '\.class' /tmp/t25.j | awk '{print $NF}')
...
echo "--- rung25: $PASS pass, $FAIL fail, $XFAIL xfail ---"
[ $FAIL -eq 0 ]
