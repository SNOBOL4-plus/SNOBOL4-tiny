#!/bin/bash
# run_rung14.sh — rung14_limit JVM corpus runner
set -euo pipefail
DRIVER="${1:-/tmp/scrip-cc}"
JASMIN="$(dirname "$0")/../../../src/backend/jvm/jasmin.jar"
CORPUS="$(dirname "$0")/corpus/rung14_limit"
TMPDIR_OUT="$(mktemp -d)"
...
run_test t01_limit_to
run_test t02_limit_alt
run_test t03_limit_zero
run_test t04_limit_large
run_test t05_limit_str
echo "--- rung14: $PASS/5 PASS, $FAIL FAIL ---"
