...
for icn in test/frontend/icon/corpus/rung18_real_relop/t*.icn; do
  ...
  java -jar src/backend/jvm/jasmin.jar /tmp/t18_x.j -d /tmp/ >/dev/null 2>&1
  cls=$(grep -m1 '\.class' /tmp/t18_x.j | awk '{print $NF}')
...
echo "--- rung18_real_relop: $PASS pass, $FAIL fail, $XFAIL xfail ---"
[ $FAIL -eq 0 ]
