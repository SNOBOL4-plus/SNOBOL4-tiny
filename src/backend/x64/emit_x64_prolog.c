 ...
 * All declarations (pl_safe_name, pl_atom_*, emit_*, emit_*,
...
static const char *pl_safe_name(const char *s) {
    if (!s) return "unknown";
    char *d = _pl_safe_buf;
    int   n = 0;
...
static char  *pl_atom_strings[MAX_ATOMS];
static char   pl_atom_labels[MAX_ATOMS][128];
static int    pl_atom_count = 0;
/* Set before emit_clause_block so emit_pl_term_load can compute correct var offsets */
static int    pl_max_ucalls = 0;
static int    pl_uid = 0;
static int    pl_next_uid(void) { return pl_uid++; }

static const char *pl_intern_atom(const char *name) {
    /* return existing label if already interned */
    for (int i = 0; i < pl_atom_count; i++)
        if (strcmp(pl_atom_strings[i], name) == 0)
            return pl_atom_labels[i];
    if (pl_atom_count >= MAX_ATOMS) return "pl_atom_overflow";
    int idx = pl_atom_count++;
...
static void emit_atom_data(void) {
    if (pl_atom_count == 0) return;
...
static void emit_atom_data_v2(void) {
    if (pl_atom_count == 0) return;
    A("\nsection .data\n");
...
/* -------------------------------------------------------------------------
 * emit_header — globals, externs, .bss trail
 * ---------------------------------------------------------------------- */
...
static void emit_pl_term_load(EXPR_t *e, int frame_base_words) {
    ...
    switch (e->kind) {
        case E_QLIT: {
            const char *lbl = pl_intern_atom(e->sval ? e->sval : "");
            A("    lea     rax, [rel %s]\n", lbl);
            break;
        }
        case E_ILIT:
        ...
        case E_VART: {
            ...
            if (slot < 0) {
                ...
                A("    mov     rax, [rbp - %d]  ; var slot %d (%s)\n",
                  (5 + pl_max_ucalls + pl_max_ucalls + slot)*8, slot, e->sval ? e->sval : "_");
            }
            break;
        }
        case E_FNC:
            if (e->nchildren == 0) {
                ...
                const char *lbl = pl_intern_atom(e->sval ? e->sval : "f");
 ...
 * emit_clause_block via the base parameter. */

static void emit_clause_block(EXPR_t *clause, int idx, int total,
        ...
        if (goal->kind == E_FNC && goal->sval) {
            ...
            if (strcmp(fn, "\\+") == 0 && garity == 1) {
                ...
                if (!ifn) {
                    ...
                    char icall_safe[256];
                    snprintf(icall_safe, sizeof icall_safe, "%s", pl_safe_name(icall_fa));
                    if (ia > 0) {
                ...
                for (int fi = 0; fi < nflat; fi++) {
                    ...
                    if (sub->kind == E_FNC && sub->sval) {
                        ...
                        if (strcmp(sfn,"nl")==0 && sa==0) {
                            ...
                            char csafe[256]; strncpy(csafe, pl_safe_name(cfa), 255);
                            char cfail[128];
            ...
            if (strcmp(fn, ";") == 0 && garity == 2) {
                ...
                if (left && left->kind == E_FNC && left->sval &&
                    ...
                    if (!cond_handled && cond && cond->kind == E_FNC && cond->sval &&
                        ...
                        if (!cifn || (strcmp(cifn,"fail")==0 && cia==0)) {
                            ...
                            char cisafe[256]; strncpy(cisafe, pl_safe_name(cifa), 255);
                            if (cia > 0) {
                    ...
                    if (!cond_handled && cond && cond->kind == E_FNC && cond->sval) {
                        ...
                        char csafe2[256]; strncpy(csafe2, pl_safe_name(cfa2), 255);
                        if (ca2 > 0) {
                            A("    sub     rsp, %d\n", ALIGN16(ca2*8));
                    ...
                    if (then && then->kind == E_FNC && then->sval) {
                        ...
                        if (strcmp(tfn,"write")==0 && ta==1) {
                            ...
                            if (!tifn||(strcmp(tifn,"fail")==0&&tia==0)){A("    mov     eax, -1\n");}
                                ...
                                char tisafe[256]; strncpy(tisafe,pl_safe_name(tifa),255);
                            ...
                            char tsafe[256]; strncpy(tsafe, pl_safe_name(tfa), 255);
                            if (ta > 0) {
                                A("    sub     rsp, %d\n", ALIGN16(ta*8));
                    ...
                    if (right && right->kind == E_FNC && right->sval) {
                        ...
                        if (strcmp(efn,"write")==0 && ea==1) {
                            ...
                            if (!eifn||(strcmp(eifn,"fail")==0&&eia==0)){A("    mov     eax, -1\n");}
                                ...
                                char eisafe[256]; strncpy(eisafe,pl_safe_name(eifa),255);
                            ...
                            char esafe[256]; strncpy(esafe, pl_safe_name(efa), 255);
                            if (ea > 0) {
                                A("    sub     rsp, %d\n", ALIGN16(ea*8));
                ...
                if (user_call_idx >= 0) {
                    ...
                    char ucsafe[256]; strncpy(ucsafe, pl_safe_name(ucfa), 255);
                    char retry_lbl[128];
                    ...
                    for (int li = user_call_idx+1; li < nlg; li++) {
                        ...
                        if (strcmp(pgn,"nl")==0) { A("    mov edi,10\n"); A("    call putchar\n"); }
                            ...
                            char isafe[256]; strncpy(isafe, pl_safe_name(ifa), 255);
                            if (pga > 0) {
                                A("    sub     rsp, %d\n", ALIGN16(pga*8));
                ...
                if (right && right->kind == E_FNC && right->sval) {
                    if (strcmp(right->sval,"true")==0) { /* no-op */ }
                        ...
                        char rsafe2[256]; strncpy(rsafe2, pl_safe_name(rfa2), 255);
                        if (ra2 > 0) {
                            A("    sub     rsp, %d\n", ALIGN16(ra2*8));
            ...
            if (strcmp(fn, ",") == 0 && garity >= 2) {
                ...
                for (int fi2 = 0; fi2 < nf2; fi2++) {
                    ...
                    if (strcmp(sfn2,"nl")==0 && sa2==0) {
                        ...
                        char csafe3[256]; strncpy(csafe3, pl_safe_name(cfa3), 255);
                        char cfail3[128];
            ...
            /* --- user-defined predicate call --- */
            {
                ...
                char call_safe[256];
                snprintf(call_safe, sizeof call_safe, "%s", pl_safe_name(call_safe_fa));
                char β_lbl[128];
...
static void emit_prolog_choice(EXPR_t *choice) {
    ...
    /* Make a stable copy of safe name (pl_safe_name uses static buffer) */
    char safe[256]; strncpy(safe, pl_safe_name(pred), 255); safe[255]='\0';
    ...
    for (int ci = 0; ci < nclauses; ci++) {
        pl_max_ucalls = max_body_ucalls;
        emit_clause_block(choice->children[ci], ci, nclauses,
...
static void emit_prolog_main(Program *prog) {
    ...
    char safe_init[256]; strncpy(safe_init, pl_safe_name(init_full), 255);
...
static void emit_prolog_program(Program *prog) {
    ...
    /* Reset atom table */
    for (int i = 0; i < pl_atom_count; i++) free(pl_atom_strings[i]);
    pl_atom_count = 0;
